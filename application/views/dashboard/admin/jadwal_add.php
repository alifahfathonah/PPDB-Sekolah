...
<body class="bg-secondary">
  ...
  <section class="side">
    <div class="container my-4">
      ...
      <form method="POST" action="<?= base_url() ?>/dashboard/jadwal/act_tambah" class="row flex-column col-md-5">
        <label class="input-container-bottom py-2 my-2">
          <div class="small">Jam Awal</div>
          <input type="time" name="jam_awal" required placeholder="Jam Awal" class="w-100" />
        </label>

        <label class="input-container-bottom py-2 my-2">
          <div class="small">Jam Akhir</div>
          <input type="time" name="jam_akhir" required placeholder="Jam Akir" class="w-100" />
        </label>
        ...
        <div class="small mt-3">Kelas</div>
        <select required name="id_kelas" class="w-100 py-2 mb-2">
          <?php foreach ($kelas as $value) : ?>
            <option value="<?= $value->id ?>"><?= $value->nama ?></option>
          <?php endforeach; ?>
        </select>

        <div class="small mt-3">Grade</div>
        <select required name="id_grade" class="w-100 py-2 mb-2">
          <?php foreach ($grade as $value) : ?>
            <option value="<?= $value->id ?>"><?= $value->nama ?></option>
          <?php endforeach; ?>
        </select>

        <div class="small mt-3">Jurusan</div>
        <select required name="id_jurusan" class="w-100 py-2 mb-2">
          <?php foreach ($jurusan as $value) : ?>
            <option value="<?= $value->id ?>"><?= $value->nama ?></option>
          <?php endforeach; ?>
        </select>

        <button class="btn btn-primary w-100 mt-4 py-2">Tambah Jadwal</button>
      </form>
    </div>
  </section>
