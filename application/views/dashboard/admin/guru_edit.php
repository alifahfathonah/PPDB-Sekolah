...
<body class="bg-secondary">
  ...
  <section class="side">
    <div class="container my-4">
      ...
      <form method="POST" action="<?= base_url() ?>/dashboard/guru/act_edit/<?= $list->id ?>" class="row flex-column col-md-5">
        <label class="input-container-bottom py-2 my-2">
          <div class="small">Nama</div>
          <img src="/static/images/icons/user.svg" />
          <input type="text" value="<?= $list->nama ?>" name="nama" required placeholder="John Doe" class="ml-2" />
        </label>

        <label class="input-container-bottom py-2 my-2">
          <div class="small">Email</div>
          <img src="/static/images/icons/mail.svg" />
          <input type="email" value="<?= $list->email ?>" name="email" required placeholder="johndoe89@gmail.com" class="ml-2" />
        </label>

        <div class="small mt-3">Status</div>
        <select required name="status" class="w-100 py-2 mb-2">
          <option value="0" <?= $list->status === '0' ? 'selected' : null ?>>Tidak aktif</option>
          <option value="1" <?= $list->status === '1' ? 'selected' : null ?>>Aktif</option>
        </select>
